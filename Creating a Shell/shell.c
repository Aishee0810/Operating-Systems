...
int Pipe(char *cd,int x)
{
    int pfd[x][2];
    //char *cmd1=(char *)malloc(100 * sizeof(char));
    //char *cmd2=(char *)malloc(100 * sizeof(char));
    int i=0,j=0,k=0,a=0,b=0,arr[100];
    char *command[x+1];
    for(i=0;i<=x;i++)
    {
        ...
        if(fork()==0)
        {
        	for(i=1;i<x;i++)
        	{
        		...
        		if(fork()!=0)
        		{
        			...
        			char *cmd1=(char *)malloc(100 * sizeof(char));
            		char *cmd2=(char *)malloc(100 * sizeof(char));
            		char *cmd3=(char *)malloc(100 * sizeof(char));
            		char *cmd4=NULL;
            		for(j=0;j<strlen(command[p-i]);j++)
            		{
                ...
                cmd1[i]=command[p-i][j];
            		}
        	if(strlen(command[p-i])!=strlen(cmd1))
            {
                cmd2 = command[p-i]+j+1;
                cmd2[strlen(cmd[p-i])-strlen(cmd1)-1] = '\0';
            }
            if(cmd2!=NULL)
            {
				...
				if(strlen(cmd2)!=strlen(cmd3))
				{
					cmd4=cmd2+j+1;
					cmd4[strlen(cmd2)-strlen(cmd3)-1]='\0';
				}
            }
            else
            	cmd3=cmd2;
            char *p[]={cmd1,cmd3,cmd4,0};
            if(execvp(p[0],p)<0)
            {
